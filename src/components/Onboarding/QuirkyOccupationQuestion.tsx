...
interface QuirkyOccupationQuestionProps {
  value: string;
  onChange: (value: string) => void;
  onNext: () => void;
}

const QuirkyOccupationQuestion: React.FC<QuirkyOccupationQuestionProps> = ({ 
  value, 
  onChange, 
  onNext 
}) => {
  const occupationOptions: OccupationOption[] = [
    { id: 'student', label: 'College Student', icon: '🎓', description: 'Surviving on ramen and dreams' },
    { id: 'professional', label: 'Working Professional', icon: '💼', description: 'Adulting like a boss' },
    { id: 'entrepreneur', label: 'Entrepreneur', icon: '🚀', description: 'Building the next big thing' },
    { id: 'freelancer', label: 'Freelancer', icon: '💻', description: 'Working in pajamas' },
    { id: 'student_job', label: 'Student with Job', icon: '📚', description: 'Balancing books and bills' },
    { id: 'other', label: 'Something Else', icon: '✨', description: 'Charting your own path' }
  ];
  
  // No state variables needed for the card-based layout
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Help us understand your lifestyle so we can find the perfect PG for you!
        </motion.p>
      </motion.div>
      ...
      >
        <motion.button
          onClick={onNext}
          disabled={!value}
          className={`group relative overflow-hidden px-6 sm:px-8 py-2 sm:py-3 rounded-full text-white text-sm sm:text-base font-medium ${
            value
...
export default QuirkyOccupationQuestion;
