  ...
  try {
    ...
    if (text && /^\d{6}$/.test(text)) {
      ...
      await sendTelegramMessage(
        chatId,
        "Welcome to Shelterly Notification Bot! 🏠\n\n" +
      ...
      await sendTelegramMessage(
        chatId,
        "I don't understand that command. To connect your account, please enter the 6-digit connection code from the Shelterly admin panel."
      );
    }
  ...
  .onCreate(async (snapshot, context) => {
    ...
    try {
      ...
      await updateNotificationStatus(
        ...
        "sent", 
        `Sent to ${successCount}/${telegramChatIds.length} Telegram chats`
      );
 ...
 */
async function handleConnectionCode(code: string, chatId: number, username: string) {
  ...
  if (requestsQuery.empty) {
    await sendTelegramMessage(
      chatId,
      "❌ Invalid or expired connection code. Please generate a new code from the Shelterly admin panel."
    );
    return;
  }
  ...
  if (diffMinutes > 15) {
    await sendTelegramMessage(
      chatId,
      "❌ Connection code has expired. Please generate a new code from the Shelterly admin panel."
    );
  ...
  try {
    ...
    await sendTelegramMessage(
      chatId,
      "✅ Successfully connected to Shelterly!\n\n" +
